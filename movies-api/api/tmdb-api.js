...
export const getImagesForMovieCards = ({ queryKey }) => {
...
export const getSimilar = async (movie_id) => {
    try {
        ...
        const response = await fetch(
            `https://api.themoviedb.org/3/movie/${movie_id}/similar?api_key=${process.env.TMDB_KEY}&language=en-US&page=1`
        );
...
export const getImagesForMovieCard = (id) => {
    return fetch(
        `https://api.themoviedb.org/3/movie/${id}/images?api_key=${process.env.TMDB_KEY}`
    ).then( (response) => {
        if (!response.ok) {
